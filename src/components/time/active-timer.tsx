...
export function ActiveTimer({ tasks, onSaveLog }: ActiveTimerProps) {
  ...
  const [elapsedForSave, setElapsedForSave] = useState(0);

  // Manual countdown state
  const [manualTotalSeconds, setManualTotalSeconds] = useState(25 * 60);
  const [manualRemainingSeconds, setManualRemainingSeconds] = useState(25 * 60);
  ...
  const [pomoTotalSeconds, setPomoTotalSeconds] = useState(DEFAULT_POMODORO_SETTINGS.focusTime * 60);
  ...
  useEffect(() => {
    if (isRunning) {
      intervalRef.current = setInterval(() => {
        if (timerType === 'Manual') {
          setManualRemainingSeconds(prev => {
            if (prev <= 1) {
              handleManualComplete();
              return 0;
            }
            totalElapsedInSession.current += 1;
            setElapsedForSave(totalElapsedInSession.current);
            return prev - 1;
          });
        } else {
  ...
  const handleManualComplete = () => {
    setIsRunning(false);
    if (startTime && totalElapsedInSession.current > 0) {
      saveSession(totalElapsedInSession.current);
    }
    totalElapsedInSession.current = 0;
    setElapsedForSave(0);
    setManualRemainingSeconds(manualTotalSeconds);
    setStartTime(null);
  };
  ...
  const handlePomodoroComplete = () => {
    ...
    if (pomoPhase === 'Focus') {
      const breakSecs = DEFAULT_POMODORO_SETTINGS.breakTime * 60;
      setPomoPhase('Break');
      setPomoTotalSeconds(breakSecs);
      setPomoRemainingSeconds(breakSecs);
    } else {
      const focusSecs = DEFAULT_POMODORO_SETTINGS.focusTime * 60;
      setPomoPhase('Focus');
      setPomoTotalSeconds(focusSecs);
      setPomoRemainingSeconds(focusSecs);
    }
    totalElapsedInSession.current = 0;
    setElapsedForSave(0);
    setStartTime(null);
  ...
  const openEditTimer = () => {
    if (isRunning) return;
    const currentSecs = timerType === 'Manual' ? manualRemainingSeconds : pomoRemainingSeconds;
  ...
  const applyEditTimer = () => {
    ...
    if (totalSecs < 60) return;
    if (timerType === 'Manual') {
      setManualTotalSeconds(totalSecs);
      setManualRemainingSeconds(totalSecs);
      totalElapsedInSession.current = 0;
      setElapsedForSave(0);
    } else {
      setPomoTotalSeconds(totalSecs);
  ...
  const displayTime = timerType === 'Manual' ? manualRemainingSeconds : pomoRemainingSeconds;

  // Progress 0% → 100% as time elapses
  const progressPercent = timerType === 'Pomodoro'
    ? ((pomoTotalSeconds - pomoRemainingSeconds) / pomoTotalSeconds) * 100
    : ((manualTotalSeconds - manualRemainingSeconds) / manualTotalSeconds) * 100;

  const activeBg = timerType === 'Manual' ? 'bg-blue-500/5' : (pomoPhase === 'Focus' ? 'bg-rose-500/5' : 'bg-emerald-500/5');
  const activeText = timerType === 'Manual' ? 'text-blue-500' : (pomoPhase === 'Focus' ? 'text-rose-500' : 'text-emerald-500');

  const activeTaskName =
    selectedTaskId && selectedTaskId !== 'unassigned' && tasks.length > 0
      ? tasks.find(t => t.id === selectedTaskId)?.title || "General Focus"
      : "General Focus";

  const canStop = isRunning || elapsedForSave > 0;
      ...
      <Card className={`overflow-hidden transition-all duration-500 backdrop-blur-xl border border-border/50 shadow-lg ${isRunning ? activeBg : 'bg-card/80'}`}>
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            ...
            <div className="flex bg-muted/50 p-1 rounded-lg">
              <Button variant={timerType === 'Pomodoro' ? 'secondary' : 'ghost'} size="sm" className={`h-7 px-3 text-xs ${timerType === 'Pomodoro' ? 'shadow-sm' : ''}`} onClick={() => handleTypeChange('Pomodoro')} disabled={isRunning}>
                Pomodoro
              </Button>
              <Button variant={timerType === 'Manual' ? 'secondary' : 'ghost'} size="sm" className={`h-7 px-3 text-xs ${timerType === 'Manual' ? 'shadow-sm' : ''}`} onClick={() => handleTypeChange('Manual')} disabled={isRunning}>
                Manual
              </Button>
            </div>
          </div>
        ...
        <CardContent className="space-y-6 pt-4">
          ...
          <div className="flex justify-center h-6">
            ...
            {timerType === 'Manual' && isRunning && (
              <div className={`flex items-center gap-1.5 text-sm font-medium ${activeText} animate-pulse`}>
                <span className="w-2 h-2 rounded-full bg-blue-500" /> Recording
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center py-2">
            {isEditingTimer ? (
              <div className="flex flex-col items-center gap-3">
                <div className="flex items-center gap-2">
                  <div className="flex flex-col items-center">
                    ...
                    <Input type="number" min="0" max="23" value={editHours} onChange={e => setEditHours(e.target.value)} className="w-20 h-12 text-center text-2xl font-bold" autoFocus />
                  </div>
                  <span className="text-3xl font-bold mt-4">:</span>
                  <div className="flex flex-col items-center">
                    <Label className="text-xs text-muted-foreground mb-1">Minutes</Label>
                    <Input type="number" min="1" max="59" value={editMinutes} onChange={e => setEditMinutes(e.target.value)} className="w-20 h-12 text-center text-2xl font-bold" onKeyDown={e => e.key === 'Enter' && applyEditTimer()} />
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                {!isRunning && <span className="text-xs text-muted-foreground">click to edit</span>}
              </div>
            )}
          </div>
          ...
          <div className="flex justify-center gap-4 pt-2">
            {!isRunning && (elapsedForSave > 0 || displayTime < (timerType === 'Pomodoro' ? pomoTotalSeconds : manualTotalSeconds)) ? (
            ...
            <Button
              className={`h-16 w-16 rounded-full shadow-lg border-2 ${isRunning ? 'bg-amber-500 hover:bg-amber-600 border-amber-400 text-white' : 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500 text-white'}`}
              onClick={toggleTimer}
            >
            ...
            {timerType === 'Manual' || pomoPhase === 'Focus' ? (
              <Button
                ...
                className={`h-12 w-12 rounded-full transition-all ${canStop ? 'opacity-100 hover:text-rose-500 hover:border-rose-500 cursor-pointer' : 'opacity-30 cursor-not-allowed'}`}
                onClick={stopAndSave}
                disabled={!canStop}
                title="Stop and Save"
              >
          ...
          {/* Manual log without timer */}
          {timerType === 'Manual' && !isRunning && (
            <div className="pt-2 border-t border-border/50">
              <Button variant="ghost" size="sm" className="w-full text-xs text-muted-foreground hover:text-foreground" onClick={() => setShowManualInput(!showManualInput)}>
                ...
                <div className="mt-3 space-y-3 p-3 rounded-lg bg-muted/30 border border-border/50">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 space-y-1">
                      ...
                      <Input type="number" min="0" max="23" value={manualHours} onChange={e => setManualHours(e.target.value)} className="h-8 text-center" />
                    </div>
                    <span className="text-lg font-bold mt-5">:</span>
                    <div className="flex-1 space-y-1">
                      <Label className="text-xs">Minutes</Label>
                      <Input type="number" min="1" max="59" value={manualMinutes} onChange={e => setManualMinutes(e.target.value)} className="h-8 text-center" />
                    </div>
                  </div>
                  <Button size="sm" className="w-full" onClick={saveManualEntry} disabled={(parseInt(manualHours) || 0) === 0 && (parseInt(manualMinutes) || 0) < 1}>
                    Save Log
                  </Button>
                </div>
              )}
      ...
      <AnimatePresence>
        {isRunning && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-xl flex flex-col items-center justify-center p-6 sm:p-12 overflow-hidden"
          >
            <motion.div
            ...
            >
              ...
              <div className="relative flex items-center justify-center mb-16 shadow-2xl rounded-full">
                <svg className="w-64 h-64 md:w-80 md:h-80 transform -rotate-90 drop-shadow-2xl">
                  ...
                  <motion.circle
                    ...
                    strokeDashoffset={`calc(${2 * Math.PI * 150} * (1 - ${Math.max(0, Math.min(100, progressPercent))} / 100))`}
                ...
                <div className="absolute flex flex-col items-center justify-center text-center">
                  ...
                  <span className="text-muted-foreground font-medium mt-2">
                    {Math.round(Math.max(0, progressPercent))}% Completed
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-6 mb-16">
                <Button
                  className={`h-20 w-20 rounded-full shadow-2xl hover:scale-105 active:scale-95 transition-all border-2 ${isRunning ? 'bg-amber-500 hover:bg-amber-600 border-amber-400 text-white' : 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500 text-white'}`}
                  onClick={toggleTimer}
                >
                ...
                {(timerType === 'Manual' || pomoPhase === 'Focus') && (
                  <Button
                    ...
                    onClick={() => { stopAndSave(); setIsRunning(false); }}
                    disabled={!canStop}
                  >
