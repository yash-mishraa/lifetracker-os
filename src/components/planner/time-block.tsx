...
export function TimeBlockItem({ block, onToggleComplete, onEdit, onDelete }: TimeBlockItemProps) {
  ...
  const getThemeVars = () => {
    ...
    switch (block.type) {
      ...
      case "habit": return "bg-orange-500/10 border-orange-500/30 text-orange-700 dark:text-orange-400";
      case "focus": return "bg-purple-500/10 border-purple-500/30 text-purple-700 dark:text-purple-400";
      case "break": return "bg-green-500/10 border-green-500/30 text-green-700 dark:text-green-400";
      ...
      <div className="flex items-stretch border-l border-inherit divide-x divide-inherit">
        {onEdit && (
          <button onClick={() => onEdit(block)}
            className="px-2.5 flex items-center justify-center bg-background/20 hover:bg-background/40 transition-colors"
            title="Edit block">
        ...
        {onDelete && (
          <button onClick={() => onDelete(block.id)}
            ...
            title="Remove block">
            <Trash2 className="h-3.5 w-3.5 opacity-50 group-hover/del:opacity-100 group-hover/del:text-red-500 transition-all" />
          </button>
        )}
      </div>
    </div>
